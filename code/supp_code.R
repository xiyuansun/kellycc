...
sc17_alter_pval <- NBsim_scenario(params=trimmed_parms,
                                  nGenes=1000, 
                                  nSample=16, 
                                  pDiff=0.3, nSim=5)

saveRDS(sc17_alter_pval, "./sim/results/sc16_alter_pval.rds")

#add eBayes
####################################################################################################################
#sc17
nGenes=1000; nSample=16; pDiff=0.3
...
saveRDS(all_pval, file=paste0("./sim/results/","sim_genes_",nGenes,"_g_",nRep, "_pDiff_",diffPerc, "_",i,"_pval",
                              ".rds"))
sc17_sim5_auc <- plot_roc_all(all_result=all_pval, name="sc17 sim5")
saveRDS(sc17_sim5_auc, file=paste0("./sim/results/","sim_genes_",nGenes,"_g_",nRep, "_pDiff_",diffPerc, "_",i,"_auc",
                                   ".rds"))
##########################################################################################################################
